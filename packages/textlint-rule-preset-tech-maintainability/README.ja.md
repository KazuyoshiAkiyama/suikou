...
違う接頭辞を持つ。詳しくは後述の「ルールID」を見る。

| ルール名 | 何を見つけるか |
|---|---|
| `list-lead-in` | 箇条書きの導入文が項目と文法的に結合していて、項目を増減すると壊れる |
...
| `trailing-etc` | 地の文または項目が「など」「etc.」で終わっている |
...
二つの実装には、作りのうえで一つだけ違う点がある。`suikou check` は原文を直接、
...
[ゴールデンテスト](../../tests/golden/)は、そのファイルごとの一致を確かめている。
...
その結果として IPADIC ではひとつの条件にまとまる。この理由付けと、それを確かめた実測は
...
導入文が項目と文法的に結合していると、下の箇条書きを直したときに壊れる。
...
`list-lead-in` は、この導入文が助詞で終わっていて完全な文でも体言止めの
...
`parallel-items` は、一方が句点で終わり、もう一方が終わっていないために
この箇条書きを指摘する。すべての項目を完全な文で終えるか、すべての項目を
言い切らない形で揃えれば指摘は消える。どちらの指摘も、箇条書きの項目を
増やしたり減らしたりしたときに文章の側が壊れるかどうかを見ている点は同じである。
...
手前が述語で終わっているかを見ており、`list-lead-in` の日本語判定に近い。判定が重なる
場面でも目的は違う。あちらは AI が書いた日本語に特有の言い回しを検出し、`list-lead-in`
は項目数が変わると壊れる箇条書きの構造を検出する。同じ行が両方から指摘されることが
...
判定が終わる前にテストが成功したことになってしまう箇所があった。
